...
const CrearEvento = () => {
  ...
  const [fechaLimite, setFechaLimite] = useState('2026-10-15');
  const [limiteDiario, setLimiteDiario] = useState('6,0');

  // --- PLAN LOGÍSTICO INICIAL (SUBTAREAS) ---
  const [subtareas, setSubtareas] = useState([
    { id: 1, nombre: 'Reservar salón', fecha: '2026-09-18', horas: '3,0' },
    { id: 2, nombre: 'Enviar invitaciones', fecha: '2026-09-20', horas: '2,0' },
  ]);
  ...
  // Enviar datos con validación tipo Figma
  const handleSubmit = async (e) => {
    ...
    // Si la validaciones pasan, limpiamos errores
    setErrores({});
    setMostrarAlerta(false);

    // Payload de envío para la API
    const payload = {
      nombre: nombreEvento,
      tipo: tipoEvento,
      fechaLimite: fechaLimite,
      limiteDiarioHoras: parseFloat(limiteDiario.replace(',', '.')),
      subtareas: subtareas.map((s) => ({
        nombre: s.nombre,
        fecha: s.fecha,
        horasEstimadas: parseFloat(s.horas.replace(',', '.'))
      }))
    };
          ...
          <div className="crear-card">
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="Lanzamiento Producto X"
                value={nombreEvento}
                onChange={(e) => {
            ...
            <div className="row-2">
              ...
              <div className="form-group">
                ...
                <input
                  type="date"
                  className={`input-text ${errores.fechaLimite ? 'input-error' : ''}`}
                  value={fechaLimite}
                  onChange={(e) => {
